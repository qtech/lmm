...
<div class="tbl">
	<div class="tbl-row">
		<div class="tbl-cell">
			<h3>Edit Featured Artist Song <label class="label label-primary">{{$song->song_name}}</label></h3>
			<small class="text-muted custom-alignment">Edit only the required information.</small>
		</div>		
		<div class="tbl-cell tbl-cell-action">
			<a href="{{route('artists.index')}}" class="btn btn-secondary pull-right"><i class="fa fa-hand-o-left"></i> Back</a>
		</div>
	</div>
</div>

<div class="box-typical box-typical-padding">
	<form action="{{route('artists.update',['id'=>$song->song_id])}}" method="POST" enctype="multipart/form-data">
	{{csrf_field()}}	
	{{method_field('PUT')}}
	<div class="form-group row">
		<label class="col-sm-3 form-control-label ajdust-label-forms">Select Album</label>
		<div class="col-sm-9">
			<p class="form-control-static">
				<select class="bootstrap-select" name="album_id">					
						...
						@empty
							<option>Please add albums.</option>
						@endforelse
				</select>
			</p>
		</div>
	...
	<div class="form-group row">
		...
		<div class="col-sm-9">
			<p class="form-control-static">
				...
				<a href="{{route('artists.index')}}" class="btn btn-danger"><i class="fa fa-times" aria-hidden="true"></i> Cancel</a>
			</p>
		</div>
	</div>
	</form>
