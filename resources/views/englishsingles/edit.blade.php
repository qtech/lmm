...
<div class="box-typical box-typical-padding">
	<form action="{{route('english.update',['id'=>$song->song_id])}}" method="POST" enctype="multipart/form-data">
	{{csrf_field()}}	
	{{method_field('PUT')}}
			...
			<p class="form-control-static">
				<select class="bootstrap-select" name="album_id">					
						@foreach($albums as $album)
							<option @if($album->featured_dj_id == $song->featured_dj_id) selected @endif value="{{$album->feature_dj_id}}">{{$album->name}}</option>							
						@endforeach
				</select>
			</p>
		</div>
